...
/**
 * \brief Callback to change the state in controlthread.c
 *
 * \param[in] msgData Data to be received.
 */


void SubscribeHandlerGameOnTopic(MessageData *msgData)
{
	StartJXGame();
...
* @brief	Send the real-time keypad input to the cloud
* @param[in]	usr: the user number
				led: the led number on keypad
				act: the action on led-> 1 for on, 0 for off
...
/**************************************************************************//**
void SendRealTimeUserGameInput(int usr, int led, int act)
* @brief	Send the answer to the cloud
* @param[in]	steps: an int array for what player one walk though
